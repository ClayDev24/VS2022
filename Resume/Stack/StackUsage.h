...
//		utils::assure(in, "main.cpp");
		...
		while(getline(in, line))
			stk.push(new std::string(line));
		in.close();
		// Pop the lines from the Stack and print them:
		while((s = (std::string*)stk.pop())) { // void* must be first cast back to a string ptr.
			std::cout << *s << "\n"; // String ptr dereferenced.
			delete s; 
		}
		stk.cleanup();
...
//		utils::assure(in2, "main.cpp");
		...
		while(getline(in2, line))
			stk2.push(new std::string(line));
		in2.close();
		// Pop the lines from the Stack and print them:
		while((s = (std::string*)stk2.pop())) { // void* must be first cast back to a string ptr.
			std::cout << *s << "\n"; // String ptr dereferenced.
			delete s; 
		}
		stk2.cleanup();
...
//		utils::assure(in3, "main.cpp");
		...
		// Read file and store lines in the Stack:
		while(getline(in2, line))
			stk3.push(new std::string(line));
		in3.close();
		// Pop the lines from the Stack and print them:
		while((s = (std::string*)stk3.pop())) { // void* must be first cast back to a string ptr.
			std::cout << *s << "\n"; // String ptr dereferenced.
